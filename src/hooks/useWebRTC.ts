...
export default function useWebRTCAudioSession(): UseWebRTCAudioSessionReturn {
  ...
  function configureDataChannel(dataChannel: RTCDataChannel) {
    ...
    const sessionUpdate = {
      ...
      session: {
        ...
        instructions:
          'You are a helpful language learning assistant. Respond naturally to user speech and help them practice their target language. Always respond with both text and audio.',
      },
    };
    ...
    const languageMessage = {
      ...
      item: {
        ...
        content: [
          {
            type: 'input_text',
            text: "Hello, I'd like to practice speaking. Can you help me?",
          },
        ],
      },
    };
    dataChannel.send(JSON.stringify(languageMessage));
    console.log('📤 Initial message sent:', languageMessage);
    ...
    console.log('📤 Response creation requested:', responseCreate);

    console.log('🚀 Initial conversation started');
  }
    ...
    try {
      ...
      switch (msg.type) {
        ...
        case 'response.audio_transcript.delta': {
          console.log('🗣️ Assistant speaking:', msg.delta);
          const newMessage: Conversation = {
        ...
        /**
         * Response completed (success or failure)
         */
        case 'response.done': {
          if (msg.response?.status === 'failed') {
            ...
            if (error?.type === 'insufficient_quota') {
              setStatus('❌ OpenAI quota exceeded. Please check your billing.');
              console.error('💳 Billing issue detected:', error.message);
            } else {
              setStatus(
  ...
  async function getEphemeralToken() {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text(); // capture error details before throwing
        console.error('Failed to get token. Response text:', errorText);
  ...
  const startSession = useCallback(async () => {
    try {
      ...
      audioEl.style.display = 'none'; // Hide the element
      audioEl.muted = false; // Ensure not muted
      audioEl.volume = 1.0; // Set volume to maximum
        ...
        if (playPromise !== undefined) {
          playPromise
            ...
            .catch((err) => {
              ...
              // If autoplay failed due to user interaction requirement, try to play on next user interaction
              if (err.name === 'NotAllowedError') {
                ...
                const resumeAudioOnClick = () => {
                  ...
                  audioEl
                    ...
                    .catch((err) => {
                      console.error('❌ Audio resume after click failed:', err);
                    });
                };
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `OpenAI Realtime failed: ${response.status} - ${errorText}`
        );
      }
  ...
  const stopSession = useCallback(() => {
    ...
    if (assistantAudioElRef.current) {
      ...
      if (assistantAudioElRef.current.parentNode) {
        assistantAudioElRef.current.parentNode.removeChild(
          assistantAudioElRef.current
        );
      }
